...
# Install /cache-timer slash command
TIMER_SKILL_DIR="$HOME/.claude/skills/cache-timer"
mkdir -p "$TIMER_SKILL_DIR"
cp "$SCRIPT_DIR/skills/cache-timer.md" "$TIMER_SKILL_DIR/SKILL.md"
echo "cache-timer: Installed /cache-timer command"

echo ""
echo "cache-timer: Installed! Restart Claude Code to activate."
echo ""
echo "  The timer is on by default. Hide it when you don't need it:"
echo "    /cache-timer     — toggle off/on for this session"
echo ""
echo "  States:"
...
echo ""
echo "  Commands:"
echo "    /cache-timer   — toggle the status line"
